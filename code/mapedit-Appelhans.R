...
drwn = drawFeatures(map = mapview(franconia))
mapview(drwn) + franconia

### edit ===================================================================
trls = editFeatures(trails[1:2, ])
mapview(trls, color = "red") + trails[1:2, ]

home = geocode_OSM(q = "7 The Spur, Christchurch, New Zealand",
                   as.sf = TRUE)

mapview(home)

home %>%
  editFeatures(map = mapview(home)) %>%
  mapview

### select =================================================================
slct = selectFeatures(franconia)
mapview(slct, col.regions = "magenta") +
  mapview(franconia, zcol = "district")

slct = selectFeatures(franconia, mode = "draw")
mapview(slct)
